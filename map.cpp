...
void map::saveMap()
{
	...
	ofstream mapFile;
	mapFile.open("test.map"); //save content to test file

	if (mapFile.is_open())
	{
		mapFile << "[Map]" <<endl;
		mapFile << "author=Sean O'Connor" << endl;
		mapFile << "image=world.bmp" << endl;
		mapFile << "wrap=no" <<endl;
		mapFile << "scroll=horizontal" << endl;
		mapFile << "warn=yes" << endl << endl;
